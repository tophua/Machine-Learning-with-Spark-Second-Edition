...
object BikeSharingExecutor {
  ...
  def main(args: Array[String]) {
    ...
    // read from csv
    val df = spark.read.format("csv").option("header", "true").load("/Users/apple/Idea/workspace/Machine-Learning-with-Spark-Second-Edition/Chapter07/scala/2.0.0/scala-spark-app/src/main/scala/org/sparksamples/regression/dataset/BikeSharing/hour.csv")
    df.cache()

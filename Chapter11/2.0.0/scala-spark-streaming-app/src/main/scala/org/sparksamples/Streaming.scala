 ...
 * network connection
  * 一个生成器应用程序，每秒生成5个随机“产品事件”，并通过网络连接发送它们
 */
 ...
 * A more complex Streaming app, which computes statistics and prints the results for each batch in a DStream
  * 一个更复杂的Streaming应用程序,它计算统计信息并打印DStream中每个批次的结果
 */
object StreamingAnalyticsApp {
  ...
  def main(args: Array[String]) {
    ...
    /*
      ...
      由于每个批次都是RDD，因此我们在DStream上调用EeachRDD，并应用常规的RDD函数
